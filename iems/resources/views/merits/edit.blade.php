            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <div class="form-container">
                        <form method="POST" action="{{ route('merits.update', $merit->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                <label for="prisoner_id" class="block text-gray-700 dark:text-gray-300">Prisoner ID:</label>
                                <input type="text" id="prisoner_id" name="prisoner_id" value="{{ $merit->prisoner_id }}" required autocomplete="prisoner_id"
                            ...
                            <div class="mb-4">
                                ...
                                <textarea id="description" name="description" required autocomplete="description"
                                    class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 dark:bg-gray-600 dark:border-gray-400 rounded-md shadow-sm focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">{{ $merit->description }}</textarea>
                                @error('description')
                                    <span class="text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="mb-4">
                                ...
                                <input type="text" id="points" name="points" value="{{ $merit->points }}" required autocomplete="points"
                                    class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 dark:bg-gray-600 dark:border-gray-400 rounded-md shadow-sm focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50">
                                @error('points')
                                    <span class="text-red-500">{{ $message }}</span>
                                @enderror
