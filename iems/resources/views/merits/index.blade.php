    ...
    <div class="py-12 bg-gray-100 dark:bg-gray-900">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <table id="meritsTable" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($merits as $merit)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-center">{{ $merit->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">{{ $merit->prisoner_id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">{{ $merit->description }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">{{ $merit->points }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex justify-center items-center space-x-2">
                                        <a href="{{ route('merits.edit', $merit->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring focus:ring-indigo-300 disabled:opacity-25 dark:bg-gray-700 dark:hover:bg-indigo-700 dark:active:bg-indigo-900 dark:text-white transition">Edit</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
